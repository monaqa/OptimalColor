...
function obj(model)
    ...
    power_bg = sum(b .^ 2) + bbase ^ 2
    power_fg = sum(f .^ 2) + fbase ^ 2
    variance_bg = (model_config.n_bg * sum(b .^ 2) - sum(b * transpose(b))) / 2
    variance_fg = (model_config.n_fg * sum(f .^ 2) - sum(f * transpose(f))) / 2
    return (power_bg + power_fg) + (variance_bg + variance_fg)
end
@objective(model, Min, obj(model))
...
for i in 1:model_config.n_fg+1
    color = @sprintf("#%02x%02x%02x", fmat[i, 1], fmat[i, 2], fmat[i, 3]);
    c = Crayon(
    background = (bmat[1, 1], bmat[1, 2], bmat[1, 3]),
    foreground = (fmat[i, 1], fmat[i, 2], fmat[i, 3]),
    );
    println(c(color))
end
